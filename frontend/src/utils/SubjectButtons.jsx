...
export const SubjectButtons = ({ _id, onSubjectDelete }) => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`${baseURL}/api/subject/${_id}`, {
        headers: {
    ...
    <>
      <div className="flex space-x-3">
        <button
          ...
          onClick={() => navigate(`/admin-dashboard/subject/${_id}`)}
        >
          Edit
        </button>
        <button
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg lg:w-[400px] w-[80%] mx-auto">
