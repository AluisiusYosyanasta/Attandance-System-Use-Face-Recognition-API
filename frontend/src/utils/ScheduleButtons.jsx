...
export const ScheduleButtons = ({ _id, onScheduleDelete }) => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`${baseURL}/api/schedule/${_id}`, {
        headers: {
    ...
    <>
      <div className="flex space-x-3">
        <button
          ...
          onClick={() => navigate(`/admin-dashboard/schedule/${_id}`)}
        >
          Edit
        </button>
        <button
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-100">
            ...
            <p className="mb-4">Apakah Anda yakin akan menghapus jadwal ini?</p>
