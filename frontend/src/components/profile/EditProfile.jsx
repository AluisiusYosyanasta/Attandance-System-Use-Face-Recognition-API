...
const EditProfile = () => {
  ...
  const dashboardBase =
    user?.role === "guru" ? "/guru-dashboard" : "/admin-dashboard";

  useEffect(() => {
    document.title = "Ubah Profile | Sistem Kehadiran";
    const fetchUser = async () => {
      try {
        const res = await axios.get(`${baseURL}/api/auth/profile/${id}`, {
          headers: {
        ...
        if (res.data.success) {
          ...
          setFormData({
            ...
            dob: data.guru.dob ? dayjs(data.guru.dob) : null,
            dob_city: data.guru.dob_city || "",
            gender: data.guru.gender || "",
            title: data.guru.title || "",
            position: data.guru.position || "",
            staGuru: data.guru.staGuru || "",
            subject: data.guru.subject?.map((sub) => sub._id) || [],
            role: data.user?.role || "",
            password: "",
          });
        }
    ...
    const fetchSubjectOptions = async () => {
      ...
      if (!Array.isArray(subs)) return; // jika bukan array, hindari error
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const redirectRole =
          formData.role === "guru" ? "guru-dashboard" : "admin-dashboard";
        navigate(`/${redirectRole}/profile/${id}`);
      }
    } catch (err) {
        ...
        <div className="max-w-4xl mx-auto mt-5 bg-white p-8 rounded-md shadow-md">
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="font-semibold text-gray-700">Nama Guru</label>
                <input
                  type="text"
                  name="name"
